...
int Graph::get_label(int e_src, int e_dst) {
    auto it = label_map.find(std::make_pair(e_src, e_dst));
    if (it == label_map.end()) {
        return 0;
    }
    return it->second;
...
void Graph::readGraph(istream& in) {
    ...
    while (getline(in, buf)) {
        // format of a line
        //     src: dst1 dst2 dst3.label3 dst4.label4 dst5 #
        strTrimRight(buf);
        idx = buf.find(":");
        ...
        while (buf.find(" ") != string::npos) {
            ...
            int dot_idx = sub.find(".");
            if (dot_idx != string::npos) {
                std::string dst_sub = sub.substr(0, dot_idx);
                std::string label_sub = sub.substr(dot_idx+1);
                sub = std::move(dst_sub);
                lid = std::stoi(label_sub);

                if (lid == 0) {
                    cerr << "label cannot be zero!" << endl;
                    exit(1);
                }
            }
...
void Graph::addEdge(int sid, int tid, int lid) {
    if (sid >= vl.size())
        addVertex(sid);
    if (tid >= vl.size())
        addVertex(tid);
    ...
    if (lid != 0) { // 0 means no label
        label_map[std::make_pair(sid, tid)] = lid;
    }
}
