...
bool Grail::go_for_reach(int src, int trg) {
    ...
    for (eit = el.begin(); eit != el.end(); eit++) {
        int next_src = *eit;
        if (visited[next_src] != QueryCnt && contains(next_src,trg)){
            // check label and add label to LabelStack
            int label = g.get_label(src, next_src);
            bool cfl_matched = (label == 0 ? true : LabelStack.add(label));

            if (cfl_matched)
                if (go_for_reach(next_src, trg)){
                    LabelStack.pop();
                    return true;
                }
        }
